...
/// `(x, y)`..`(x+w, y+h)`.
pub fn draw_chart_raster(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), text: (u8, u8, u8)) {
...
fn rect(buf: &mut [u8], w: usize, h: usize, x0: usize, y0: usize, x1: usize, y1: usize, c: (u8, u8, u8)) {
    for y in y0..y1 {
        for x in x0..x1 {
            set(buf, w, h, x, y, c);
        }
...
fn bar(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    let n = spec.values.len().max(1);
    let max = max_value(&spec.values);
    let plot_w = cw.saturating_sub(16);
    let plot_h = ch.saturating_sub(32);
    let bar_w = (plot_w / n).min(48).max(2);
    ...
    for (i, v) in spec.values.iter().enumerate() {
        ...
        let by = y + plot_h - bh;
        rect(buf, w, h, bx, by, bx + bar_w, y + plot_h, if i % 2 == 0 { a1 } else { a2 });
    }
}

fn line(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    ...
    for (px, py) in &pts {
        rect(buf, w, h, px.saturating_sub(2), py.saturating_sub(2), px + 2, py + 2, a2);
    }
}

fn pie(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    ...
    for (i, v) in spec.values.iter().enumerate() {
        ...
        for s in 0..=samples {
            ...
            if let Some((lx, ly)) = prev {
                draw_triangle(buf, w, h, cx as isize, cy as isize, lx, ly, px as isize, py as isize, color);
            }
    ...
    if is_donut {
        ...
        rect(buf, w, h, cx.saturating_sub(hole), cy.saturating_sub(hole), cx + hole, cy + hole, (11, 15, 25));
    }
}

fn draw_triangle(buf: &mut [u8], w: usize, h: usize, ax: isize, ay: isize, bx: isize, by: isize, cx: isize, cy: isize, color: (u8, u8, u8)) {
...
fn point_in_tri(px: isize, py: isize, ax: isize, ay: isize, bx: isize, by: isize, cx: isize, cy: isize) -> bool {
...
fn scatter(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    let n = spec.values.len().max(2);
    let max = max_value(&spec.values);
    let plot_w = cw.saturating_sub(16);
    ...
    for (i, v) in spec.values.iter().enumerate() {
        ...
        let py = y + plot_h - ((v / max) * plot_h as f64) as usize;
        let color = if i % 2 == 0 { a1 } else { a2 };
        rect(buf, w, h, px.saturating_sub(2), py.saturating_sub(2), px + 3, py + 3, color);
    }
}

fn heatmap(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    let max = max_value(&spec.values);
    let n = spec.values.len().max(1);
    ...
    for (i, v) in spec.values.iter().enumerate() {
        ...
        let cx = x + (i % 5) * cell_w;
        let cy = y + (i / 5) * cell_h;
        let c = ((color.0 as u16 * intensity as u16 / 255).min(255) as u8, (color.1 as u16 * intensity as u16 / 255).min(255) as u8, (color.2 as u16 * intensity as u16 / 255).min(255) as u8);
        rect(buf, w, h, cx, cy, cx + cell_w - 4, cy + cell_h - 4, c);
    }
}

fn gauge(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    let max = max_value(&spec.values);
    let cx = x + cw / 2;
    let cy = y + ch;
...
fn stacked(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    let n = spec.values.len().max(1);
    let max = max_value(&spec.values);
    let plot_w = cw.saturating_sub(16);
    let plot_h = ch.saturating_sub(32);
...
fn area(spec: &ChartSpec, buf: &mut [u8], w: usize, h: usize, x: usize, y: usize, cw: usize, ch: usize, a1: (u8, u8, u8), a2: (u8, u8, u8), _t: (u8, u8, u8)) {
    let n = spec.values.len().max(2);
    let max = max_value(&spec.values);
    let plot_w = cw.saturating_sub(16);
    let plot_h = ch.saturating_sub(16);
        ...
        for pair in pts.windows(2) {
            ...
            if xc >= lo && xc <= hi {
                ...
                let yy = if hi == lo {
                    ...
                    (y0 as isize + (y1 as isize - y0 as isize) * (xc as isize - lo as isize) / span as isize) as usize
                };
                if yy < top {
                    top = yy;
                }
