...
pub fn regions_px(layout: &LayoutDef, width: u32, height: u32) -> Vec<RegionRect> {
    layout
        ...
        .map(|r| {
            ...
            RegionRect { slot: slot_of(&r.slot), x, y, w, h }
        })
        .collect()
}

fn rect_for<'a>(regions: &'a [RegionRect], slot: Slot) -> Option<&'a RegionRect> {
    regions.iter().find(|r| r.slot == slot)
}

/// Theme color roles (bg, cardBg, accent1, accent2, text).
fn colors(theme: PaletteTheme) -> (&'static str, &'static str, &'static str, &'static str, &'static str) {
    theme.colors()
}
...
pub fn render_svg_regions(layout: &LayoutDef, spec: &InfographicLayoutSpec) -> Option<String> {
    ...
    svg.push_str(&format!("    .metric-val {{ font-size: 22px; font-weight: 800; fill: {}; }}\n", accent1));
    svg.push_str("    .metric-lbl { font-size: 10px; font-weight: 600; fill: #9CA3AF; letter-spacing: 0.5px; }\n");
    svg.push_str(&format!("    .badge {{ font-size: 11px; font-weight: 800; fill: {}; }}\n", bg));
    svg.push_str("  </style>\n");
    svg.push_str(&format!(
    ...
    if let Some(r) = rect_for(&regions, Slot::Chart) {
        if let Some(chart) = &spec.chart {
            let c = crate::chart::ChartColors { bg, card_bg, accent1, accent2, text };
            svg.push_str(&crate::chart::ChartGlyphRenderer::render(chart, &c, r.x, r.y, r.w, r.h));
        }
    }
