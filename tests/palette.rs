...
fn all_theme_variants_resolve_colors() {
    ...
    for theme in [
        ...
        assert!(c.bg.starts_with('#') && c.bg.len() == 7, "{theme:?} bg {:?}", c.bg);
...
fn corpus_colors_match_fallback_for_existing_themes() {
    ...
    for theme in [
        ...
        assert_eq!((c.bg, c.card_bg, c.accent1, c.accent2, c.text), (bg, card, a1, a2, text));
    }
}
...
fn new_themes_differ_from_existing() {
    ...
    for theme in [PaletteTheme::OceanBreeze, PaletteTheme::SunsetGlow, PaletteTheme::ForestMint, PaletteTheme::Monochrome] {
        let c = reg.colors(theme);
...
fn theme_classification_reaches_new_palettes() {
    ...
    assert_eq!(r.parse_and_route("Ocean breeze report").theme, PaletteTheme::OceanBreeze);
    assert_eq!(r.parse_and_route("Sunset glow campaign poster").theme, PaletteTheme::SunsetGlow);
    assert_eq!(r.parse_and_route("Forest eco sustainability poster").theme, PaletteTheme::ForestMint);
    assert_eq!(r.parse_and_route("Minimal monochrome print").theme, PaletteTheme::Monochrome);
}
