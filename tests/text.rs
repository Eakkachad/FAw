...
fn text_renderer_draws_into_buffer() {
    ...
    assert!(
        non_bg > 100,
        "text rasterization must produce visible pixels, got {non_bg}"
    );
}
...
fn png_is_deterministic_with_text() {
    ...
    assert_eq!(
        a, b,
        "PNG output must be byte-identical (font rasterization is deterministic)"
    );
}
...
fn thai_text_rasterizes_without_panic() {
    ...
    assert!(
        changed > 50,
        "Thai text should rasterize visible glyphs, got {changed}"
    );
}
