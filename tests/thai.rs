...
fn thai_layout_classification() {
    ...
    assert_eq!(
        r.parse_and_route("สร้างไทม์ไลน์การพัฒนาระบบ 4 ขั้นตอน")
            .layout_type,
    ...
    assert_eq!(
        r.parse_and_route("เปรียบเทียบฟีเจอร์ระหว่างสองผลิตภัณฑ์")
            .layout_type,
...
fn thai_theme_classification() {
    let r = InfographicIntentRouter::new();
    assert_eq!(
        r.parse_and_route("รายงานการเงินสีน้ำเงินเข้ม").theme,
        PaletteTheme::FinancialNavy
    );
    assert_eq!(
        r.parse_and_route("โปสเตอร์ธรรมชาติสีเขียว").theme,
        PaletteTheme::ForestMint
    );
    assert_eq!(
        r.parse_and_route("โปสเตอร์สร้างสรรค์สีปะการัง").theme,
        PaletteTheme::VibrantCoral
    );
    assert_eq!(
        r.parse_and_route("เอกสารวิชาการโทนทอง").theme,
...
fn thai_aspect_classification() {
    ...
    assert_eq!(
        r.parse_and_route("สร้างแบนเนอร์ไทม์ไลน์แนวนอน").aspect_ratio,
        AspectRatio::Banner16_9
    );
    // square → Square1_1
    assert_eq!(
        r.parse_and_route("โพสต์สี่เหลี่ยมจัตุรัส").aspect_ratio,
...
fn thai_numeric_step_count() {
    ...
    assert_eq!(
        spec.sections.len(),
        4,
        "Thai word numeral should yield 4 sections"
    );
}
...
fn thai_mixed_english_prompt() {
    ...
    assert!(
        spec.chart.is_some(),
        "mixed TH/EN chart prompt should bind a chart"
    );
}
